...
<body class="bg-gray-50 min-h-screen font-sans">
    ...
    <nav class="bg-white border-b border-gray-200 sticky top-0 z-10">
        <div class="max-w-xl mx-auto px-4 py-3 flex items-center justify-between">
            ...
            <a href="/posts/{{ $post->id }}" class="text-sm text-gray-500 hover:text-gray-800">&larr; Back to post</a>
        </div>
    </nav>

    <div class="max-w-xl mx-auto px-4 py-6">
        <h1 class="text-lg font-semibold text-gray-800 mb-4">Edit Post</h1>

        <div class="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
            <form method="POST" action="/posts/{{ $post->id }}" class="space-y-3">
                @csrf
                @method('PUT')
                ...
                <textarea name="body" placeholder="Body" rows="4"
                    class="w-full border border-gray-200 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-gray-400">{{ old('body', $post->body) }}</textarea>
                @error('body')
                <p class="text-red-500 text-xs">{{ $message }}</p>
                @enderror
